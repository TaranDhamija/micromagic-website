...
function HeroCtaButton() {
  ...
  return (
    <a
      href={waLink(
        "Hi MicroMagic! I'd like to order. Can you help me choose the right product?"
      )}
      target="_blank"
      ...
      <div className="relative z-10 mx-auto grid min-h-screen w-full max-w-6xl grid-cols-1 gap-16 px-6 pb-14 pt-28 lg:grid-cols-[minmax(0,1.2fr)_minmax(300px,0.8fr)] lg:items-center lg:pt-32">
        <div className="max-w-[720px]">
          ...
          <p className="mt-7 max-w-[540px] font-display text-[clamp(19px,2.4vw,25px)] leading-relaxed text-[#D8D5C7]">
            Pure herbal powders and botanicals for a slower,
            steadier kind of wellness. Honest ingredients,
            prepared with care, designed to become part of
            everyday life.
          </p>
          ...
          <div className="mt-14 grid max-w-[560px] grid-cols-3 gap-5 border-t border-cream/10 pt-8">
            {statItems.map((item) => (
              <div key={item.label}>
                ...
                {/* LABEL — CHANGED TO LIGHTER COLOR */}
                <span className="text-[11px] uppercase tracking-[1.8px] text-cream/75">
                  {item.label}
                </span>
