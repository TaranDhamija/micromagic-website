            ...
            {product.whyPeopleChoose.map((item, index) => (
              <div
                ...
                className={`reveal reveal-delay-${Math.min(
                  index + 1,
                  4
                )} rounded-[28px] border border-forest/8 bg-warmwhite p-7 shadow-[0_12px_36px_rgba(18,35,25,0.05)]`}
              >
            ...
            {product.dailyBenefits.map((benefit, index) => (
              <div
                ...
                className={`reveal reveal-delay-${Math.min(
                  index + 1,
                  4
                )} rounded-[26px] border border-sage/14 bg-white p-6`}
              >
                <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-sage/12 text-sage">
                  ✦
                </div>

                <p className="text-sm leading-relaxed text-textmid">
                  {benefit}
                </p>
              </div>
            ))}
            ...
            {product.howToUseCards.map((item, index) => (
              <div
                ...
                className={`reveal reveal-delay-${Math.min(
                  index + 1,
                  4
                )} rounded-[28px] border border-forest/8 bg-white p-7 shadow-[0_12px_36px_rgba(18,35,25,0.05)]`}
              >
                <p className="mb-3 text-[10px] uppercase tracking-[2.3px] text-sage font-medium">
                  {item.title}
                </p>

                <p className="font-display text-[26px] italic leading-[1.14] text-textmid">
                  {item.body}
                </p>
              </div>
            ))}
            ...
            {product.specifications.map(([label, value], index) => (
              <div
                ...
                className={`reveal reveal-delay-${Math.min(
                  index + 1,
                  4
                )} rounded-[24px] border border-forest/8 bg-warmwhite px-5 py-5`}
              >
                <p className="mb-2 text-[10px] uppercase tracking-[2.2px] text-textlight">
                  {label}
                </p>
            ...
            {product.certifications.map((item, index) => (
              <div
                ...
                className={`reveal reveal-delay-${Math.min(
                  index + 1,
                  4
                )} rounded-[24px] border border-gold/16 bg-white px-5 py-6 text-center shadow-[0_12px_36px_rgba(18,35,25,0.04)]`}
              >
            ...
            {product.whyChooseMicroMagic.map((item, index) => (
              <div
                ...
                className={`reveal reveal-delay-${Math.min(
                  index + 1,
                  4
                )} rounded-[26px] border border-forest/8 bg-parchment/72 px-6 py-5`}
              >
            ...
            {product.beyondDailyUse.map((item, index) => (
              <div
                ...
                className={`reveal reveal-delay-${Math.min(
                  index + 1,
                  4
                )} rounded-[30px] border border-forest/8 bg-warmwhite p-7 shadow-[0_12px_36px_rgba(18,35,25,0.05)]`}
              >
                <p className="mb-3 text-[10px] uppercase tracking-[2.3px] text-sage font-medium">
                  {item.title}
                </p>

                <p className="font-display text-[28px] italic leading-[1.12] text-textmid">
                  {item.body}
                </p>
              </div>
            ))}
      ...
      <section className="py-20 px-6 bg-parchment">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="font-serif text-3xl text-forest mb-3">
              Continue exploring
            </h2>
          ...
          <div className="reveal reveal-delay-1">
            ...
            <div className="grid gap-5 md:grid-cols-[minmax(0,0.7fr)_minmax(0,1fr)] mb-10">
              <div className="rounded-[28px] border border-forest/8 bg-white p-6 shadow-[0_14px_45px_rgba(18,35,25,0.05)]">
                ...
                <p className="font-serif text-4xl text-forest">
                  {product.price}
                </p>

                <p className="mt-2 text-sm text-textlight">
                  {product.weight} · Small batch packed
                </p>
              </div>

              <div className="rounded-[28px] border border-sage/15 bg-sage/8 p-6">
                <p className="text-[10px] uppercase tracking-[2px] text-sage font-medium mb-2">
                  Product Story
                </p>
              ...
              <div className="rounded-[28px] border border-forest/8 bg-white p-7 shadow-[0_14px_45px_rgba(18,35,25,0.05)]">
                ...
                <ul className="space-y-3">
                  {product.benefits.map((benefit, index) => (
                    <li
                      key={index}
                      className="flex items-start gap-3 text-sm leading-relaxed text-textmid"
                    >
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-3">
            {product.faqs.map((faq, index) => (
              ...
              >
                ...
                >
                  <span className="font-sans font-medium text-forest text-sm">
                    {faq.q}
                  </span>

                  <span
                    className={`text-sage text-lg flex-shrink-0 transition-transform duration-300 ${
                      openFaq === index
                        ? 'rotate-45'
                        : ''
                    }`}
                ...
                >
                  <p className="px-6 pb-5 text-sm text-textmid leading-relaxed">
                    {faq.a}
                  </p>
                </div>
              </div>
          ...
          <div className="text-center mt-10">
            <p className="text-textlight text-sm mb-4">
              Want guidance before you order?
            </p>

            <a
              href={waLink(
                `Hi MicroMagic! I have a question about ${product.name}.`
              )}
              target="_blank"
              rel="noopener noreferrer"
...
export default function ProductDetail() {
  ...
  // Wheatgrass Pouch and any legacy Wheatgrass slug
  // use the dedicated Wheatgrass experience.
  if (
    product.slug === 'wheatgrass-powder-pouch' ||
    product.slug === 'wheatgrass-powder'
  ) {
    return (
      <WheatgrassExperience product={product} />
    );
  }

  return (
    <GenericProductExperience product={product} />
  );
}